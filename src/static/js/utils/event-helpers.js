...
(function (global) {
    ...
    const EventHelpers = {
        ...
        dispatchCustomEvent(eventName, detail = {}, options = {}) {
            ...
            try {
                ...
                // Modern browsers: CustomEvent constructor
                ...
                // IE11 fallback: createEvent + initCustomEvent
                ...
                // Unsupported browser
                    ...
                    if (window.GeoLeaf && GeoLeaf.Log && GeoLeaf.Log.warn) {
                        GeoLeaf.Log.warn(`[EventHelpers] CustomEvent not supported, cannot dispatch '${eventName}'`);
                    }
                    return false;
                }
                ...
                if (window.GeoLeaf && GeoLeaf.Log && GeoLeaf.Log.error) {
                    GeoLeaf.Log.error(`[EventHelpers] Failed to dispatch event '${eventName}':`, error);
                }
                return false;
            }
        },
        ...
        dispatchMapEvent(map, eventName, detail = {}) {
            if (!map || typeof map.fire !== "function") {
                if (window.GeoLeaf && GeoLeaf.Log && GeoLeaf.Log.warn) {
                    GeoLeaf.Log.warn(`[EventHelpers] Invalid map instance, cannot fire '${eventName}'`);
                }
                return false;
            }
            ...
            try {
                ...
                if (window.GeoLeaf && GeoLeaf.Log && GeoLeaf.Log.error) {
                    GeoLeaf.Log.error(`[EventHelpers] Failed to fire map event '${eventName}':`, error);
                }
                return false;
            }
        },
        ...
        dispatchBoth(eventName, detail = {}, map = null) {
            ...
            const fullEventName = eventName.startsWith('geoleaf:')
                ? eventName
                : `geoleaf:${eventName}`;
        ...
        addEventListener(target, eventName, handler, options = {}) {
            if (!target || typeof target.addEventListener !== "function") {
                if (window.GeoLeaf && GeoLeaf.Log && GeoLeaf.Log.warn) {
                    GeoLeaf.Log.warn(`[EventHelpers] Invalid target for addEventListener '${eventName}'`);
                }
                return () => {}; // noop cleanup
            }
            ...
            return () => {
                try {
                    ...
                    if (window.GeoLeaf && GeoLeaf.Log && GeoLeaf.Log.error) {
                        GeoLeaf.Log.error(`[EventHelpers] Failed to remove listener '${eventName}':`, error);
                    }
                }
            };
        },
    ...
    global.GeoLeaf.Utils.EventHelpers = EventHelpers;

    // Convenient aliases at root level
    global.GeoLeaf.dispatchEvent = EventHelpers.dispatchCustomEvent.bind(EventHelpers);
    global.GeoLeaf.dispatchMapEvent = EventHelpers.dispatchMapEvent.bind(EventHelpers);

    // Utility aliases (Sprint 3.2 - consolidated from helpers.js)
    global.GeoLeaf.Utils.debounce = EventHelpers.debounce;
    global.GeoLeaf.Utils.throttle = EventHelpers.throttle;
