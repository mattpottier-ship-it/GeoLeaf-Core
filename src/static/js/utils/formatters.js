...
(function (global) {
     ...
     */
    function formatDistance(distanceInMeters, options = {}) {
        const {
     ...
     */
    function toGB(bytes, precision = 2) {
     ...
     */
    function toMB(bytes, precision = 1) {
        if (!bytes || bytes === 0) return "0";
    ...
    // Alias sur GeoLeaf.Utils pour facilité d'usage
    GeoLeaf.Utils.formatDistance = formatDistance;
    GeoLeaf.Utils.formatDate = formatDate;
    GeoLeaf.Utils.formatDateTime = formatDateTime;
    GeoLeaf.Utils.formatNumber = formatNumber;
    GeoLeaf.Utils.formatFileSize = formatFileSize; // Alias for formatBytes compatibility

    // Backward compatibility with FormatUtils (DEPRECATED)
    GeoLeaf.Utils.FormatUtils = {
        formatBytes: formatFileSize, // Redirect to formatFileSize
        toGB,
        toMB,
        formatPercent: (value, precision = 0) => formatPercentage(value / 100, { precision }), // Convert 0-100 to 0-1
        formatDuration: (seconds) => formatDuration(seconds * 1000), // Convert seconds to ms
        formatDate: (date) => formatDateTime(date, { style: 'short' }),
        formatNumber,
        getFilenameFromURL,
        truncate,
        capitalize,
    ...
    if (GeoLeaf.Log && GeoLeaf.Log.info) {
        GeoLeaf.Log.info('[GeoLeaf.Utils.Formatters] Module loaded (includes deprecated FormatUtils for backward compatibility)');
    }
