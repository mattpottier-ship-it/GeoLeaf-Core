...
(function (global) {
    ...
    const LegendModule = {
        ...
        init: function (mapInstance, options) {
            ...
            if (Log) Log.info("[Legend] Module Legend initialisé avec génération automatique depuis styles");
            return true;
        },
        ...
        _rebuildDisplay: function () {
            ...
            if (_control && typeof _control.updateMultiLayerContent === "function") {
                ...
                if (hasIcons) {
                    ...
                    if (!sprite) {
                        if (Log) Log.info("[Legend] Icônes détectées mais sprite manquant - programmation retry");
        ...
        /**
         * @deprecated Utiliser loadLayerLegend à la place
         */
        addLayerLegend: function (layerId, styleId, legendData) {
            if (Log) Log.warn("[Legend] addLayerLegend() est déprécié avec données JSON, utiliser loadLayerLegend avec style");
            // Ajouter quand même pour compatibilité
            const layerInfo = _allLayers.get(layerId);
            if (layerInfo) {
                layerInfo.legendData = legendData;
                layerInfo.styleId = styleId;
        ...
        /**
         * @deprecated Utiliser setLayerVisibility à la place
         */
        removeLayerLegend: function (layerId) {
            if (Log) Log.warn("[Legend] removeLayerLegend() est déprécié, utiliser setLayerVisibility(id, false)");
        ...
        updateLayerLegend: function (layerId, styleId, legendData) {
            if (Log) Log.warn("[Legend] updateLayerLegend() avec JSON est déprécié, utiliser loadLayerLegend");
        ...
        loadLegend: function (legendFilePath) {
            if (Log) Log.warn("[Legend] loadLegend() est déprécié");
        ...
        showLegend: function (legendData) {
            if (Log) Log.warn("[Legend] showLegend() est déprécié");
        },
