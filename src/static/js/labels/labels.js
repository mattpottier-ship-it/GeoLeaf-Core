...
(function (global) {
    ...
    const Labels = {
        ...
        _calculateMapScale(map) {
            ...
            if (Log) {
                Log.debug(`[Labels] Calcul échelle (fallback): zoom=${zoom}, lat=${center.lat.toFixed(2)}, échelle=1:${scale.toLocaleString()}`);
            }
        ...
        _isScaleInRange(currentScale, minScale, maxScale) {
            ...
            if (typeof minScale === "number" && currentScale > minScale) {
                if (Log) {
                    Log.debug(`[Labels] Test échelle (fallback): ${currentScale} > minScale ${minScale} → ✗ CACHÉ (trop dézoomé)`);
                }
                return false;
            }
            ...
            if (typeof maxScale === "number" && currentScale < maxScale) {
                if (Log) {
                    Log.debug(`[Labels] Test échelle (fallback): ${currentScale} < maxScale ${maxScale} → ✗ CACHÉ (trop zoomé)`);
                }
                return false;
            }

            if (Log) {
                Log.debug(`[Labels] Test échelle (fallback): ${currentScale} dans [${maxScale} - ${minScale}] → ✓ VISIBLE`);
            }
