...
(function (global) {
    ...
    class SectionOrchestrator {
        ...
        getFieldValue(poi, fieldPath) {
            ...
            if (Log && Log.info) {
                Log.info('[POI] getFieldValue:', fieldPath, '→',
        ...
        async renderSection(section, poi, state) {
            ...
            if (isEmpty && !isRequiredField) {
                if (Log) Log.warn('[POI] Section ignorée (valeur vide):', section.label || section.type);
                return null;
            }

            if (Log) Log.info('[POI] → Appel render pour:', section.label, '- Type:', section.type, '- Value:',
            ...
            switch (section.type) {
                ...
                case 'number': // Afficher un nombre simple comme du texte
                    ...
                    break;
                case 'metric': // Nombre avec suffixe/préfixe
            ...
            if (Log) Log.debug('[POI] Contenu généré pour:', section.label || section.type, '- Accordion:', section.accordion);
