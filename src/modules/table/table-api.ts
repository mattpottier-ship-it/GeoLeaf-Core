...
const TableModule: any = {
    ...
    _sortState: {
        ...
        direction: null as string | null, // 'asc' | 'desc' | null
    },
    ...
    init(options: any) {
        ...
        if (_TablePanel && typeof _TablePanel.create === "function") {
            this._container = _TablePanel.create(this._map, this._config) as HTMLElement;
        } else {
    ...
    _attachMapEvents() {
        ...
        this._map.on("geoleaf:geojson:visibility-changed", (e: any) => {
    ...
    sortByField(field: any) {
        this._sortState = nextSortState(this._sortState as any, field);
        this.refresh();
     ...
     */
    setSelection(ids: any, add = false) {
        if (!add) {
            this._selectedIds.clear();
        }

        ids.forEach((id: any) => this._selectedIds.add(String(id)));
    ...
    zoomToSelection() {
        ...
        if (!L || typeof L.latLngBounds !== "function") {
            Log.warn("[Table] Leaflet (L) non disponible pour zoomToSelection");
            return;
        }
    ...
    highlightSelection(active: any) {
        ...
        if (!L || typeof L.circleMarker !== "function" || typeof L.geoJSON !== "function") {
            Log.warn("[Table] Leaflet (L) non disponible pour highlightSelection");
            return;
        }
                ...
                if (feature.geometry) {
                    ...
                    if (geomType === "Point") {
                        ...
                        const highlightLayer = L.geoJSON(feature, {
                            ...
                            interactive: false,
                            pointToLayer: function (_f: any, latlng: any) {
    ...
    _clearHighlightLayers() {
        this._highlightLayers.forEach((layer: any) => {
            try {
     ...
     * @private
     */
    _resolveFeatureId(feature: any, syntheticIndex: any) {
     ...
     * @private
     */
    _extendBoundsFromGeometry(bounds: any, geometry: any) {
     ...
     * @private
     */
    _getNestedValue(obj: any, path: any) {
     ...
     * @private
     */
    _fireEvent(eventName: any, detail: any) {
