...
import { POICoreContract } from "../../contracts/poi-core.contract.ts";
...
function getPoiBaseConfig() {
    ...
    try {
        ...
        if (ConfigAny && typeof ConfigAny.getActiveProfile === "function") {
            const activeProfile = ConfigAny.getActiveProfile();
            const poiCfg =
                activeProfile && activeProfile.appearance && activeProfile.appearance.poi;
...
function resolveCategoryDisplay(poi: any) {
    ...
    if (subCategoryId && resolvedCatKey && categoriesConfig[resolvedCatKey]?.subcategories) {
        const subCat =
            categoriesConfig[resolvedCatKey].subcategories[subCategoryId] ||
        ...
        if (categoryId && categoryId !== "undefined" && categoryId !== "null" && Log) {
            Log.warn(
                `[POI] resolveCategoryDisplay() : Catégorie '${categoryId}' non trouvée dans la taxonomie.`
            );
        }
    }
...
async function ensureProfileSpriteInjectedSync(): Promise<void> {
    ...
    try {
        const ConfigAny = Config as any;
        if (typeof ConfigAny === "undefined" || typeof ConfigAny.getIconsConfig !== "function") {
            if (Log) Log.warn("[POI] Config.getIconsConfig non disponible");
            return;
        }

        const iconsCfg = ConfigAny.getIconsConfig();
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (svgEl && svgEl.tagName.toLowerCase() === "svg") {
                    ...
                    if (Log)
                        Log.info(
                            "[POI] Sprite SVG profil injecté dans le DOM (async).",
                            symbolCount,
                            "symboles chargés."
                        );
                }
...
function attachMarkerEvents(marker: any, poi: any) {
    ...
    if (showPopup) {
        ...
        marker.on("click", function (e: any) {
            ...
            const _gPOI: any = typeof globalThis !== "undefined" ? globalThis : window;
            if (_gPOI.GeoLeaf?.POI?.showPoiDetails) {
                _gPOI.GeoLeaf.POI.showPoiDetails(poi);
            } else {
                POICoreContract.showPoiDetails?.(poi);
