...
const UIBehaviors = {
    ...
    attachSingleAccordionBehavior(container: any) {
        ...
        if (Log)
            Log.info("[POI] Comportement singleAccordion activé:", accordions.length, "accordéons");
    },
    ...
    attachGalleryEvents(sidePanelElement: any, lightboxManager: any) {
        ...
        const galleryImages = Array.from(thumbs).map((thumb: any) => thumb.querySelector("img").src);
