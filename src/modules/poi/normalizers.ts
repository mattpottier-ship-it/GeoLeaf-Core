...
function normalizePoi(poi: any) {
    ...
    const normalized = {
        ...
        label: escapeHtml(String(resolveField(p, "label", "name", "title") ?? "")),
        name: escapeHtml(String(resolveField(p, "name", "label", "title") ?? "")),
        ...
        attributes: {
            ...
            mainImage: sanitizeUrl(
                attr.photo ||
                    attr.mainImage ||
                    props.photo ||
                    (attr.gallery && attr.gallery[0]) ||
            ...
            tags: (attr.tags || p.tags || props.tags || []).map((t: any) => escapeHtml(String(t))),
            ...
            services: (attr.services || p.services || props.services || []).map((s: any) =>
                escapeHtml(String(s))
            ),
        },
    };
    ...
    if (normalized.attributes.openingHours && Array.isArray(normalized.attributes.openingHours)) {
        normalized.attributes.openingHoursTable = normalized.attributes.openingHours
            .map((raw: any) => {
    ...
    const out = normalized as any;
    if (poi._sidepanelConfig) {
        out._sidepanelConfig = poi._sidepanelConfig;
    }
    if (poi._layerConfig) {
        out._layerConfig = poi._layerConfig;
    }
...
function getFieldValue(normalizedPoi: any, fieldPath: string) {
    ...
    if (Log && typeof Log.debug === "function") {
        Log.debug(
            "[POI.getFieldValue] Recherche fieldPath:",
            fieldPath,
            "| POI:",
            normalizedPoi.id
        );
    ...
    for (const part of parts) {
        if (value == null || typeof value !== "object") {
            if (Log && typeof Log.debug === "function") {
                Log.debug(
                    "[POI.getFieldValue] Chemin interrompu à la partie:",
                    part,
                    "| value actuelle:",
                    value
                );
