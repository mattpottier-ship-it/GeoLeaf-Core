...
// Dépendances lazy — fallback pour tests (résolution module Jest)
const _defaultState = {
    options: {
        defaultStyle: {},
        defaultPointStyle: {},
 ...
 */
LayerConfigManager.resolveDataFilePath = function (dataFile: any, profile: any, layerDirectory: any) {
...
LayerConfigManager.buildLayerOptions = function (def: any, baseOptions: any) {
    ...
    const PaneHelpers = GeoJSONShared?.PaneHelpers ?? { getPaneName: (z: number) => `geoleaf-layer-${z || 0}` };
    const paneName = PaneHelpers.getPaneName(def.zIndex);

    // Style des points
    if (showIconsOnMap) {
        // Mode ICÔNE : utiliser les icônes SVG du profil
        mergedOptions.pointToLayer = function (feature: any, latlng: any) {
