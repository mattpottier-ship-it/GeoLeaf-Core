    ...
    async loadVectorTileLayer(layerId: any, layerLabel: any, def: any, _baseOptions: any) {
        ...
        try {
            if (def.styles && def.styles.default) {
                styleData = await (_g.GeoLeaf &&
                    _g.GeoLeaf._GeoJSONLayerConfig &&
                    _g.GeoLeaf._GeoJSONLayerConfig.loadDefaultStyle(layerId, def));
            }
        } catch (err) {
            Log.warn(
        ...
        const paneName = (PaneHelpers as any).getOrCreatePane
        ...
        if (vtConfig.featureIdProperty) {
            vtOptions.getFeatureId = function (feature: any) {
