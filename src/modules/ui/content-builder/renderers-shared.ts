 ...
 * Fonctions de rendu individuelles r�utilisables par tous les contextes.
 *
...
function renderText(poi, config, options = {}) {
    ...
    const _context = options.context || "popup";
    ...
    if (variant === "title") {
        ...
        if (options.includeIcon && options.resolveCategoryDisplay) {
            ...
            if (displayConfig && displayConfig.iconId) {
                const iconsConfig =
                    Config && typeof Config.getIconsConfig === "function"
                ...
                iconHtml =
                    ...
                    (displayConfig.colorFill || "#666") +
                    '" stroke="' +
                    (displayConfig.colorStroke || "#222") +
                    '" stroke-width="1.5"/>' +
...
/**
 * Construit un �l�ment de type "metric" (KPI, statistiques avec suffixe/pr�fixe)
 * @param {Object} poi - Donn�es du POI
 * @param {Object} config - Configuration de l'�l�ment
...
/**
 * Construit un �l�ment de type "rating" (note avec �toiles)
 * @param {Object} poi - Donn�es du POI
 * @param {Object} config - Configuration de l'�l�ment
 * @param {Object} options - Options de rendu
 * @returns {string} HTML
...
function renderRating(poi, config, options = {}) {
    ...
    // Affichage de la note num�rique
...
function renderBadge(poi, config, _options = {}) {
    ...
    // R�solution du label (taxonomy) et des couleurs (depuis styleRules de la couche)
...
/**
 * Construit un �l�ment de type "reviews"
 * @param {Object} poi - Donn�es du POI
 * @param {Object} config - Configuration de l'�l�ment
 * @param {Object} options - Options de rendu
 * @returns {string} HTML
...
function renderReviews(poi, config, _options = {}) {
    ...
    value.forEach((review) => {
        ...
        if (reviewAuthor || reviewDate) {
            ...
            if (reviewDate) {
                html +=
                    '<span class="gl-content__review-date">' + escapeHtml(reviewDate) + "</span>";
            }
            html += "</div>";
        }
...
function renderItem(poi, config, options = {}) {
    ...
    if (!renderer) {
        getLog().warn("[ContentBuilder.Shared] Type de rendu non support�:", config.type);
        return "";
    }
...
if (getLog().debug) {
    getLog().debug("[ContentBuilder.Shared] Renderers partag�s charg�s");
}
