...
FilterPanelShared.getBaseRoutes = function () {
    ...
    try {
        if (GeoJSONCore && typeof GeoJSONCore.getFeatures === "function") {
            const feats =
                GeoJSONCore.getFeatures({
                    geometryTypes: ["line", "linestring", "multilinestring"],
                }) || [];
...
FilterPanelShared.getFilterPanelElement = function () {
    ...
    const Log = getLog();
    Log.warn(
        "[GeoLeaf.UI.FilterPanel] Aucun conteneur de panneau de filtres trouvé (#gl-filter-panel / #gl-left-panel)."
    );
