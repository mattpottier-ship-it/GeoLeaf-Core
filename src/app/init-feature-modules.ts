...
export function initPOI({ GeoLeaf, cfg, map, AppLog }: any) {
    ...
    try {
        if (typeof poiApi.init === "function") {
            ...
            if (
                ...
                cfg.layers.forEach(function (layerRef: any) {
    ...
    const bounds: any[] = [];
    cfg.poi.forEach(function (poiItem: any) {
        let latlng = null;
 ...
 * @param {InitDeps} deps
 */
export function initRoute({ GeoLeaf, cfg, map, AppLog }: any) {
...
export function initGeoJSON({ GeoLeaf, _cfg, map, AppLog, _app }: any) {
    ...
    if (map && typeof map.on === "function") {
        map.on("geoleaf:geojson:layers-loaded", function (event: any) {
