...
**Module:** `src/static/js/labels/label-button-manager.js`  
**Version:** 3.2.0  
...
const button = GeoLeaf._LabelButtonManager.createButton(
  'poi-restaurants',
  controlsContainer
);
```
...
```html
<button class="gl-layer-manager__label-toggle gl-layer-manager__label-toggle--disabled" 
        type="button" 
        disabled
...
**File:** `src/static/js/ui/layer-manager.js`
...
function _renderLayerItem(layerId) {
  ...
  if (GeoLeaf._LabelButtonManager) {
    const labelButton = GeoLeaf._LabelButtonManager.createButton(
      layerId, 
      controls
    );
  }
...
**File:** `src/static/js/labels/label-style-loader.js`
...
describe('LabelButtonManager', () => {
  test('button enabled when layer visible and label.enabled', () => {
    ...
    expect(button.classList.contains('gl-layer-manager__label-toggle--disabled'))
      .toBe(false);
  });
});
```
...
```javascript
// Get button
const button = document.querySelector('[data-layer-id="poi-restaurants"] .gl-layer-manager__label-toggle');
