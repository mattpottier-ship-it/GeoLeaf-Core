...
GeoLeaf JS is a powerful JavaScript mapping library built on top of [Leaflet](https://leafletjs.com/), designed to simplify the creation of business-oriented mapping applications. It provides a high-level API for managing POIs (Points of Interest), GeoJSON layers, themes, filters, and plugin integrations.
...
- **ðŸ’¾ Offline Cache (Premium Plugin)** - IndexedDB-based caching for offline functionality
...
**âš ï¸ Important:** As of v3.1, `visibleByDefault` **must** be in the style file, not in the layer configuration. See [Labels documentation](labels/GeoLeaf_Labels_README.md) for details.
...
**Purpose:** Manage offline cache (IndexedDB storage) via the premium Storage plugin.

> **License note:** `GeoLeaf.Cache.*` and related offline features require the commercial Storage plugin. They are not part of the MIT core alone.
...
5. Check [Labels documentation](labels/GeoLeaf_Labels_README.md) if upgrading from older versions
