        ...
        <script>
            ...
            searchInput.addEventListener("input", (e) => {
                ...
                searchTimeout = setTimeout(async () => {
                    ...
                    if (results.length > 0) {
                        const bounds = L.latLngBounds(results.map((poi) => [poi.lat, poi.lng]));
...
async function showCacheInfo() {
    ...
    if (cacheInfoDiv) {
        ...
        document.getElementById("cache-layers").textContent = `Layers: ${info.layers.length}`;
    }
}
...
async function exportCSV() {
    ...
    const rows = pois.map((poi) => [poi.id, poi.name, poi.type, poi.lat, poi.lng, poi.description]);
